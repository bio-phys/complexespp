...
void simd_lennard_jones() {
  ...
  VecType epsilon = -2;
  SIMD_EXPECT_DOUBLE_EQ(VecType, epsilon, simd::util::lennardJones<VecType>(
                                              r0 * r0, epsilon, sigma));
  ...
  SIMD_EXPECT_DOUBLE_EQ(VecType, 0., simd::util::lennardJones<VecType>(
                                         VecType(1.), epsilon, sigma));
  ...
  SIMD_EXPECT_TRUE(simd::util::lennardJones<VecType>(VecType(std::pow(3, 2)),
                                                     epsilon, sigma) > 0);
  SIMD_EXPECT_DOUBLE_EQ(VecType, 2 * epsilon, simd::util::lennardJones<VecType>(
                                                  VecType(1.), epsilon, sigma));
...
void simd_smooth() {
  ...
  const VecType sigma = 1;
  SIMD_EXPECT_NEAR(VecType, 0, simd::util::smooth<VecType>(
                                   VecType(5), sigma, rmin_pow2, rmax_pow2),
                   1E-12);
  SIMD_EXPECT_NEAR(VecType, 1, simd::util::smooth<VecType>(
                                   VecType(1), sigma, rmin_pow2, rmax_pow2),
                   1E-12);
