...
template <class T>
class vec : public io::AbstractSerializable {
 private:
...
class Array : public io::AbstractSerializable {
 public:
  ...
  explicit Array(std::size_t rows_, std::size_t cols_,
      ...
      : m_rows(static_cast<int>(rows_)),
        m_cols(static_cast<int>(cols_)),
...
// Quick Matrix type
template <class T>
using Matrix = FixedArray<T, 3, 3>;

}  // namespace util

// TODO: use an actual linalg library like blaze/eigen/xtensor for this
template <class T>
util::vec<T> operator-(const util::vec<T>& b) {
